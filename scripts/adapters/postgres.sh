...
#
# Implements the db_* interface consumed by backup.sh / restore.sh:
...
PSQL="psql --host=$DB_HOST --port=$DB_PORT --username=$DB_USER --dbname=$DB_DATABASE"
...
# ── db_backup ──────────────────────────────────────────────────
# Args: $1 = backup_path,
#       $2 = include_tables (comma-separated; empty = all),
#       $3 = exclude_tables (comma-separated; empty = none)
...
# ── db_restore ─────────────────────────────────────────────────
# Args: $1 = backup_path, $2 = include_tables (comma-separated; empty = full restore)
#
# Two distinct paths:
...
#
# Full-restore note: the empty-include path resets the public schema
...
#   - dump integrity is verified by restore.sh (checksums) BEFORE this runs;
