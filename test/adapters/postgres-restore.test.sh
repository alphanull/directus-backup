...
#   - DB_PORT is passed to psql/pg_restore instead of being hardcoded
#
...
ADAPTER="$SCRIPT_DIR/../../scripts/adapters/postgres.sh"
...
export DB_HOST=localhost DB_PORT=15432 DB_USER=u DB_PASSWORD=p DB_DATABASE=d
...
run_restore() {
  bp="$1"; inc="$2"
  sh -c '( set -e; . "$1"; db_init; db_restore "$2" "$3" )' \
...
grep -q "\-\-disable-triggers" "$MOCK_LOG" 2>/dev/null \
  && pass "pg_restore called with --disable-triggers" || fail "pg_restore missing --disable-triggers"
grep -q "\-\-port=15432" "$MOCK_LOG" 2>/dev/null \
  && pass "uses configured DB_PORT" || fail "configured DB_PORT not used"
