#!/bin/sh
# Regression test for UPLOADS_DIR / EXTENSIONS_DIR path decoupling in the runner.
#
...
#   - direct extension folders are archived as packages except node_modules,
...
BACKUPSH="$SCRIPT_DIR/../../scripts/backup.sh"
RESTORESH="$SCRIPT_DIR/../../scripts/restore.sh"

if [ ! -f "$BACKUPSH" ] || [ ! -f "$RESTORESH" ]; then
  echo "backup.sh / restore.sh not found under $SCRIPT_DIR/../../scripts"
  exit 1
fi
...
mkdir -p "$SRC_UP/sub" "$SRC_EXT/.registry" "$SRC_EXT/myext/dist" "$SRC_EXT/myext/src" "$SRC_EXT/myext/scripts" "$SRC_EXT/myext/node_modules/dep"
...
echo "src"    > "$SRC_EXT/myext/src/should-be-archived.js"
echo "script" > "$SRC_EXT/myext/scripts/backup.sh"
echo "meta"   > "$SRC_EXT/myext/extension.config.js"
echo "dep"    > "$SRC_EXT/myext/node_modules/dep/index.js"
...
env PATH="$MOCK_BIN:$PATH" \
  ...
  sh "$BACKUPSH" > "$WORK/backup.log" 2>&1
RC=$?
...
echo "$EXT_LISTING" | grep -q "myext/src/should-be-archived.js" && pass "src/ file archived" || fail "src/ file missing from archive"
echo "$EXT_LISTING" | grep -q "myext/scripts/backup.sh" && pass "scripts/ file archived" || fail "scripts/ file missing from archive"
echo "$EXT_LISTING" | grep -q "myext/extension.config.js" && pass "arbitrary extension file archived" || fail "arbitrary extension file missing from archive"
echo "$EXT_LISTING" | grep -q "node_modules" && fail "node_modules was archived" || pass "node_modules excluded"
...
mkdir -p "$DST_UP" "$DST_EXT/myext"

# Plant stale files that must not survive the restore.
echo "stale" > "$DST_UP/.stale-dotfile"
echo "stale" > "$DST_EXT/myext/stale.txt"
echo "stale" > "$DST_EXT/myext/.stale-dotfile"

# restore.sh is the boot-time restore: arm a flag in a separate flag dir, then
# run it with DB_* / CACHE_* from the environment (as Directus provides them).
...
DB_ADAPTER=postgres
UPLOADS_DIR=$DST_UP
EXTENSIONS_DIR=$DST_EXT
RESTORE_INCLUDE_DB=0
RESTORE_INCLUDE_ASSETS=1
RESTORE_INCLUDE_EXTENSIONS=1
...
env PATH="$MOCK_BIN:$PATH" \
  ...
  sh "$RESTORESH" > "$WORK/restore.log" 2>&1
RC=$?
[ "$RC" -eq 0 ] && pass "restore.sh exits 0" || fail "restore.sh exit code $RC"
[ -f "$FLAGDIR/.restore_done" ] && pass ".restore_done marker written" || fail ".restore_done marker missing"
...
# Extensions landed directly in DST_EXT with package files, but no node_modules.
...
[ -f "$DST_EXT/myext/src/should-be-archived.js" ] && pass "myext/src restored" || fail "myext/src missing"
[ -f "$DST_EXT/myext/scripts/backup.sh" ] && pass "myext/scripts restored" || fail "myext/scripts missing"
[ -f "$DST_EXT/myext/extension.config.js" ] && pass "myext arbitrary file restored" || fail "myext arbitrary file missing"
[ ! -e "$DST_EXT/myext/node_modules" ] && pass "myext/node_modules not restored" || fail "myext/node_modules restored"
[ ! -e "$DST_EXT/myext/stale.txt" ]    && pass "stale extension file removed"   || fail "stale extension file survived"
[ ! -e "$DST_EXT/myext/.stale-dotfile" ] && pass "stale extension dotfile removed" || fail "stale extension dotfile survived"
